...
bool gxGraphics::restore()
{
	...
	if(dir3dDev) dir3dDev->EvictManagedResources();
#endif
...
static HRESULT CALLBACK enumDevice(char* desc, char* name, D3DCAPS9* devDesc, void* context)
{
	gxGraphics* g = (gxGraphics*)context;
	int t = 0;
	D3DDEVTYPE dCaps = devDesc->DeviceType;
	if(guid == IID_IDirect3DRGBDevice) t = 1;
	else if(dCaps == D3DDEVTYPE_HAL) t = 2;
...
gxMesh* gxGraphics::createMesh(int max_verts, int max_tris, int flags)
{
	...
	//XP or less? Bro, we are in 2020, how is someone gonna try to play the game
	//in windows xp or less?
	/*if(runtime->osinfo.dwMajorVersion < 6)
	{
		vbflags |= D3DVBCAPS_WRITEONLY;
	}*/

	//D3DVERTEXBUFFER_DESC desc = { sizeof(desc),vbflags,VTXFMT,max_verts };

	IDirect3DVertexBuffer9* buff;

	dir3dDev->CreateVertexBuffer(3 * sizeof(max_verts),
								 0,
								 VTXFMT,
								 D3DPOOL_MANAGED,
								 &buff,
