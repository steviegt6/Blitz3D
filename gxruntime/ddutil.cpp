...
static void adjustTexSize(int* width, int* height, IDirect3D9* dir3d)
{
	if(dir3d->GetDeviceCaps(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, gx_graphics->d3d9Caps) < 0)
	{
		*width = *height = 256;
		return;
	}
...
//    if( ddDesc.dpcTriCaps.dwTextureCaps & D3DPTEXTURECAPS_POW2 ){
	...
	//	}
		//make square
	if(gx_graphics->d3d9Caps->TextureCaps & D3DPTEXTURECAPS_SQUAREONLY)
	{
		if(w > h) h = w;
		else w = h;
	}
	//check aspect ratio
	if(max = gx_graphics->d3d9Caps->MaxTextureAspectRatio)
	{
		int asp = w > h ? w / h : h / w;
		if(asp > max)
		{
	...
	if((min = ddDesc.dwMinTextureHeight) && h < min) h = min;   WTF WAS MARK SIBLY ON???*/
	if((max = gx_graphics->d3d9Caps->MaxTextureWidth) && w > max) w = max;
	if((max = gx_graphics->d3d9Caps->MaxTextureHeight) && h > max) h = max;
...
ddSurf* ddUtil::createSurface(int w, int h, int flags, gxGraphics* gfx)
{
	...
	if(flags & gxCanvas::CANVAS_TEXTURE)
	{
		...
		adjustTexSize((int*)&desc.dwWidth, (int*)&desc.dwHeight, gfx->dir3d);
	}
	else
	{
...
ddSurf* ddUtil::loadSurface(const std::string& f, int flags, gxGraphics* gfx)
{
	...
	if(flags & gxCanvas::CANVAS_TEXTURE) adjustTexSize(&t_w, &t_h, gfx->dir3d);
